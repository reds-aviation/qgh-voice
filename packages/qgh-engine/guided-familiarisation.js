(function initializeQghGuidedFamiliarisation(root) {
  ...
  function tourSteps(kind) {
    ...
    const voice = {
      ...
      text: 'Open VOICE to prepare offline recognition. Hold PTT for one complete controller call, then release; Continuous Listening is optional. Pilot sound starts muted. For replies, wear headphones, select HEADPHONES · PILOT READBACKS, choose 100, 130 or 170 words per minute, complete the audio test and confirm. Never use pilot replies on speakers. Open VOICE to mute before disconnecting headphones. PTT can interrupt an old reply; Continuous Listening pauses until the reply finishes, then resumes automatically.',
