(function expose(root, factory) {
  ...
  function parse(value, options = {}, Voice = DefaultVoice) {
    ...
    if (correction.length === 2) {
      ...
      if (family && !/\b(?:qnh|qfe|squawk|wind|runway|temperature)\b/.test(correction[0].slice(family[0].length))) {
      ...
      else if (/^(?:climb|descend)\b/.test(correction[0]) && /^(?:climb|descend)\b/.test(correction[1])) body = correction[1];
    ...
    if (/^(?:confirm|check) altimeter(?: setting)?$/.test(body)) return accept([{ type: 'report-pressure' }]);
    if (/^confirm squawk$/.test(body)) return accept([{ type: 'report-squawk' }]);
  ...
  function speech(text) {
    ...
    return text.toLowerCase().replace(/\b(qnh|qfe)\b/g, value => value.split('').join(' '))
