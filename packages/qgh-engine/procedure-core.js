(function expose(root, factory) {
  ...
  function environment(input = {}) {
    return {
      ...
      weather: { windDirectionDeg: 0, windSpeedKt: 0, windGustKt: 0, visibilityM: 10000,
        cloudLayers: [], temperatureC: 20, dewPointC: 10, trend: 'no change', ...input.weather },
  ...
  function apply(state, plan, context = {}) {
    ...
    try {
        ...
        switch (action.type) {
          ...
          case 'weather':
            for (const [key, value] of Object.entries(action.values)) {
              ...
              if (limits[key]) env.weather[key] = bounded(value, ...limits[key], key);
              else if (key === 'cloudLayers') {
                env.weather.cloudLayers = value.map(layer => {
                  if (!['few', 'scattered', 'broken', 'overcast'].includes(layer.cover)) throw new Error('Unknown cloud cover.');
                  return { cover: layer.cover, baseFt: bounded(layer.baseFt, 0, 60000, 'Cloud base feet') };
                });
              } else if (key === 'trend' && ['no change', 'improving', 'deteriorating'].includes(value)) env.weather.trend = value;
          ...
          case 'vertical-rate':
            a.verticalRateFpm = bounded(action.value, 100, 6000, 'Vertical rate'); readbacks.push(`VERTICAL RATE ${a.verticalRateFpm} FEET PER MINUTE`); break;
          case 'report-level':
      ...
      const verticalChange = plan.actions.some(action => ['vertical', 'stop-vertical'].includes(action.type));
      if (verticalChange) {
        ...
        for (const id of affected) for (const other of Object.values(draft.aircraft)) {
          ...
          const member = draft.aircraft[id];
          const before = state.aircraft[id].actualAltitudeMslFt - state.aircraft[other.id].actualAltitudeMslFt;
          const after = member.clearedAltitudeMslFt - other.clearedAltitudeMslFt;
          if (Math.abs(after) < 1000 || before * after < 0) conflicts.push(other.callsign);
        }
