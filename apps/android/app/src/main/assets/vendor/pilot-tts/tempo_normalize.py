...
def normalize_pcm(pcm, words, ffmpeg, max_factor=2.0):
    if not 2.0 <= max_factor <= 2.2:
        raise ValueError('Tempo maximum must stay within the reviewed 2.0–2.2 range.')
    target = round(words * 60 / TARGET_WPM * SAMPLE_RATE)
    factor = len(pcm) / target
    if not 0.5 <= factor <= max_factor:
    ...
    for attempt in range(8):
        ...
        if abs(drift) <= MAX_DRIFT and tail_is_silent:
            ...
            return output.astype('<i2'), {'sourceSamples': len(pcm), 'nominalFactor': original_factor,
                                          ...
                                          'passes': attempt + 1, 'maxFactor': max_factor}
...
def normalize_banks(input_dir, output_dir, recipe_path, ffmpeg):
    ...
    metadata['segmentMaxFactorOverrides'] = {
    ...
    for voice in recipe['voices']:
        ...
        for count, segment in enumerate(recipe['segments'], 1):
            ...
            adjusted, proof = normalize_pcm(pcm, segment['words'], ffmpeg, segment.get('tempoMaxFactor', 2.0))
    ...
    result['version'] = 'qgh-pilot-kokoro-clips-en-3'
