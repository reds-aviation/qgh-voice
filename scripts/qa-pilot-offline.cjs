...
(async () => {
  ...
  try {
    ...
    await page.waitForFunction(()=>QGHRadioWorkspace.status().phase==='idle',{},{timeout:15000});
    await page.waitForFunction(()=>document.querySelector('.voice-status')?.textContent!=='PILOT TRANSMITTING',{},{timeout:5000});
